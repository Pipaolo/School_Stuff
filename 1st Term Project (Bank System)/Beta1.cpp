...
class User 
{
    ...
    public:
        void registerUser()
        {
            ...
            cin >> passInputCheck;
            cout << endl;
            std::ofstream data; //initialize file stream
        ...
        {
            int tempCount = 0;
            std::ifstream userCountRead;
            std::ofstream userCount;
            userCountRead.open("counter");

            if (!userCountRead.is_open()) // Create counter file if not found
            {
                userCount.open ("counter");
                userCount << count;
                userCount.close();  
            } else {
                userCountRead >> tempCount; //assign current value of counter to tempcount
                userCountRead.close();
